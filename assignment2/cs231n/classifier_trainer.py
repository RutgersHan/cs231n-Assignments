...
class ClassifierTrainer(object):
  ...
  def train_new(self, X, y, X_val, y_val, 
        model, loss_function, 
        reg=0.0,
        learning_rate=1e-2, momentum=0, learning_rate_decay=0.95,
        update='momentum', sample_batches=True,
...
for it in xrange(num_iters):
  ...
  for p in model:
    ...
    # update the parameters
    if not p in extreme_history: 
      extreme_history[p] = []
    extreme_history[p].append((model[p].min(),model[p].max())
    if not p in update_extreme_history: 
      update_extreme_history[p] = []
    update_extreme_history[p].append((dx.min(),dx.max()))
    model[p] += dx
...
if verbose:
  print 'finished optimization. best validation accuracy: %f' % (best_val_acc, )
# return the best model and the training history statistics
return best_model, loss_history, train_acc_history, val_acc_history,extreme_history,update_extreme_history
