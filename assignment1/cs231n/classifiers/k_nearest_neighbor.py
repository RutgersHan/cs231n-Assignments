...
class KNearestNeighbor:
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        #####################################################################
        pass
        #dist = np.absolute(X[i] - self.X_train[j])  L1 norm
        #dist = np.sum(dist)
        dist = np.linalg.norm(X[i]-self.X_train[j])  # L2 norm
        dists[i,j] = dist
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      closest_y = self.y_train[indexs[0:k]]
      #########################################################################
      # TODO:                                                                 #
      ...
      #########################################################################
      pass
      y_pred[i],count = stats.mode(closest_y)
